...
const SelectUserRecipeIcon = ({recipe, className, setSelectedRecipe, path, active}) => {
      ...
      <>
        ...
        <div className='recipe-icon__bottom'>
          ...
          <div className='reicpe-icon__row'>
            <FontAwesomeIcon icon={faSquarePlus} className='rotateicon'/>
          </div>
        </div>
      </>
    )
  ...
  return (
    <div className={`recipe-icon user-recipe-icon recipe-select-icon ${className ? className : ''}`} onClick={recipeSelected}>
      {content}
...
export default SelectUserRecipeIcon